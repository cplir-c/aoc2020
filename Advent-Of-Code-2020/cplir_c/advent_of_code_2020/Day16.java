...
public class Day16 {
    ...
    public static void main(String[] args) {
        countInvalidTicketValues(INPUT);
        countInvalidTicketValues(EXAMPLE);
        solveDeparture(INPUT);
        solveDeparture(EXAMPLE);
    }

    private static long solveDeparture(String notesString) {
        Object2ObjectMap<String, BitSet> fieldRules     = new Object2ObjectOpenHashMap<>();
        var                              validSomewhere = new BitSet();
        ObjectList<short[]>              nearbyTickets  = new ObjectArrayList<>();
        var                              myTicket       = parseNotes(notesString, fieldRules, validSomewhere, nearbyTickets);

        removeInvalidTickets(validSomewhere, nearbyTickets);

        ObjectList<ObjectSet<String>> possibleFieldsNames = new ObjectArrayList<>(fieldRules.size());
        initializePossibleFieldNames(fieldRules, possibleFieldsNames);

        nearbyTickets.add(myTicket);
        eliminateImpossibleFieldNames(fieldRules, nearbyTickets, possibleFieldsNames);

        var certainNames = deduceFieldNames(fieldRules, possibleFieldsNames);
        System.out.println(Arrays.toString(certainNames));
        var multipliedValues = 1L;
        for (var i = (short) (certainNames.length - 1); i >= 0; --i) {
            var name = certainNames[i];
            if (name.startsWith("departure")) {
                var myValue = myTicket[i];
                multipliedValues *= myValue;
            }
        }
        System.out.println(
            "ticket " + Arrays.toString(myTicket) + " contained a departure product of " + multipliedValues + "."
        );
        return multipliedValues;
    }

    private static String[] deduceFieldNames(Object2ObjectMap<String, BitSet> fieldRules,
                                             ObjectList<ObjectSet<String>> possibleFieldsNames) {
        Object2ObjectMap<String, ShortSet> fieldPossibleNames = new Object2ObjectOpenHashMap<>(fieldRules.size());
        initializeFieldPossibleNames(possibleFieldsNames, fieldPossibleNames);
        ShortStack toCheckFieldList = new ShortArrayList(fieldRules.size());
        ShortSet   toCheckFieldSet  = new ShortOpenHashSet(fieldRules.size());
        var        certainNames     = new String[fieldRules.size()];
        for (var i = (short) (certainNames.length - 1); i >= 0; --i) {
            toCheckFieldSet.add(i);
            toCheckFieldList.push(i);
        }
        while (!toCheckFieldList.isEmpty()) {
            var checkingFieldIndex = toCheckFieldList.popShort();
            toCheckFieldSet.remove(checkingFieldIndex);
            var possibleFieldNames = possibleFieldsNames.get(checkingFieldIndex);
            if (possibleFieldNames == null || possibleFieldNames.size() != 1) {
                continue;
            }

            var checkingFieldName       = possibleFieldNames.iterator().next();
            var checkingFieldReferences = fieldPossibleNames.get(checkingFieldName);
            var it                      = checkingFieldReferences.iterator();
            while (it.hasNext()) {
                var checkingFieldReferenceIndex = it.nextShort();
                var checkingFieldReferencingSet = possibleFieldsNames.get(checkingFieldReferenceIndex);
                checkingFieldReferencingSet.remove(checkingFieldName);
                if (toCheckFieldSet.add(checkingFieldReferenceIndex)) {
                    toCheckFieldList.push(checkingFieldReferenceIndex);
                }
            }
            checkingFieldReferences.clear();
            fieldPossibleNames.remove(checkingFieldName);
            possibleFieldsNames.set(checkingFieldIndex, null);
            certainNames[checkingFieldIndex] = checkingFieldName;
        }
        return certainNames;
    }

    private static void initializeFieldPossibleNames(ObjectList<ObjectSet<String>> possibleFieldsNames,
                                                     Object2ObjectMap<String, ShortSet> fieldPossibleNames) {
        for (short index = 0; index < possibleFieldsNames.size(); ++index) {
            var names = possibleFieldsNames.get(index);
            for (String name : names) {
                fieldPossibleNames.computeIfAbsent(name, _$ -> new ShortOpenHashSet()).add(index);
            }
        }
    }

    private static void eliminateImpossibleFieldNames(Object2ObjectMap<String, BitSet> fieldRules,
                                                      ObjectList<short[]> nearbyTickets,
                                                      ObjectList<ObjectSet<String>> possibleFieldsNames) {
        ObjectSet<String> impossibleFieldNames = new ObjectOpenHashSet<>();
        for (short[] ticket : nearbyTickets) {
            for (short i = 0; i < ticket.length; ++i) {
                var possibleFieldNames = possibleFieldsNames.get(i);
                var value              = ticket[i];
                for (String possibleFieldName : possibleFieldNames) {
                    var possibleFieldValues = fieldRules.get(possibleFieldName);
                    if (!possibleFieldValues.get(value)) {
                        impossibleFieldNames.add(possibleFieldName);
                    }
                }
                possibleFieldNames.removeAll(impossibleFieldNames);
                impossibleFieldNames.clear();
            }
        }
    }

    private static void initializePossibleFieldNames(Object2ObjectMap<String, BitSet> fieldRules,
                                                     ObjectList<ObjectSet<String>> possibleFieldNames) {
        var allFieldNames = new ObjectOpenHashSet<>(fieldRules.keySet());
        for (var i = allFieldNames.size(); i > 1; --i) {
            possibleFieldNames.add(allFieldNames.clone());
        }
        possibleFieldNames.add(allFieldNames);
    }

    private static void removeInvalidTickets(BitSet validSomewhere, ObjectList<short[]> nearbyTickets) {
        ObjectSet<short[]> invalidTickets = new ObjectOpenCustomHashSet<>(ShortArrays.HASH_STRATEGY);
        for (short[] nearbyTicket : nearbyTickets) {
            for (short nearbyValue : nearbyTicket) {
                if (!validSomewhere.get(nearbyValue)) {
    ...
    static final Pattern SECTION_SPLIT = Pattern.compile("\n{2,}");
    static final Pattern LINE_SPLIT    = Pattern.compile("\n+");
    static final Pattern LABEL_SPLIT   = Pattern.compile(":");
    static final Pattern SPACE_SPLIT   = Pattern.compile("\\s+");
    static final Pattern NUMBER_SPLIT  = Pattern.compile("(?:-|,)+");

    private static short countInvalidTicketValues(String notesString) {
        Object2ObjectMap<String, BitSet> fieldRules     = new Object2ObjectOpenHashMap<>();
        var                              validSomewhere = new BitSet();
        ObjectList<short[]>              nearbyTickets  = new ObjectArrayList<>();
        var                              myTicket       = parseNotes(notesString, fieldRules, validSomewhere, nearbyTickets);

        short invalidValueSum = 0;
        for (short[] nearbyTicket : nearbyTickets) {
            for (short nearbyValue : nearbyTicket) {
                if (!validSomewhere.get(nearbyValue)) {
                    invalidValueSum += nearbyValue;
                }
            }
        }
        System.out.println("sum of invalid values: " + invalidValueSum);
        return invalidValueSum;
    }

    private static short[] parseNotes(String notesString, Object2ObjectMap<String, BitSet> fieldRules, BitSet validSomewhere,
                                      ObjectList<short[]> nearbyTicketsList) {
        var sections = SECTION_SPLIT.split(notesString);

        var fieldsString          = sections[0];
        var myTicketSectionString = sections[1];
        var otherTicketsString    = sections[2];

        parseTicketFields(fieldRules, validSomewhere, fieldsString);
        parseNearbyTickets(nearbyTicketsList, otherTicketsString);
        return parseMyTicket(myTicketSectionString);
    }

    private static short[] parseMyTicket(String myTicketSectionString) {
        var myTicketSection = LABEL_SPLIT.split(myTicketSectionString);
        var myTicketLabel   = myTicketSection[0];
        var myTicketString  = myTicketSection[1];
        if (!"your ticket".equals(myTicketLabel)) {
            throw new AssertionError();
        }
        var myTicketLines = LINE_SPLIT.split(myTicketString);
        for (String myTicketLine : myTicketLines) {
            if (isFalse(myTicketLine)) {
                continue;
            }
            var myTicketNumbersStrings = NUMBER_SPLIT.split(myTicketLine);
            var myTicket               = new short[myTicketNumbersStrings.length];
            for (var i = 0; i < myTicket.length; ++i) {
                var myTicketNumberString = myTicketNumbersStrings[i];
                var myTicketNumber       = Short.parseShort(myTicketNumberString);
                myTicket[i] = myTicketNumber;
            }
            return myTicket;
        }
        throw new AssertionError(Arrays.toString(myTicketLines));
    }

    private static void parseNearbyTickets(ObjectList<short[]> nearbyTicketsList, String otherTicketsString)
            throws AssertionError {
        var otherTickets      = LABEL_SPLIT.split(otherTicketsString);
        var nearbyTicketLabel = otherTickets[0];
        if (!"nearby tickets".equals(nearbyTicketLabel)) {
            throw new AssertionError(nearbyTicketLabel);
        }
        var nearbyTicketsString = otherTickets[1];
        var nearbyTickets       = LINE_SPLIT.split(nearbyTicketsString);
        for (String nearbyTicketString : nearbyTickets) {
            if (isFalse(nearbyTicketString)) {
                continue;
            }
            var nearbyTicketStrings = NUMBER_SPLIT.split(nearbyTicketString);
            var nearbyTicket        = new short[nearbyTicketStrings.length];
            nearbyTicketsList.add(nearbyTicket);
            for (var i = 0; i < nearbyTicket.length; ++i) {
                var ticketNumberString = nearbyTicketStrings[i];
                var ticketNumber       = Short.parseShort(ticketNumberString);
                nearbyTicket[i] = ticketNumber;
            }
        }
    }

    private static void parseTicketFields(Object2ObjectMap<String, BitSet> fieldRules, BitSet validSomewhere,
                                          String fieldsString) {
        var fields = LINE_SPLIT.split(fieldsString);
        for (String fieldLineString : fields) {
            var fieldLine         = LABEL_SPLIT.split(fieldLineString);
            var fieldName         = fieldLine[0];
            var fieldValuesString = fieldLine[1];
            var validFieldValues  = new BitSet();
            fieldRules.put(fieldName, validFieldValues);
            var fieldValues = SPACE_SPLIT.split(fieldValuesString);
            for (String validRangeString : fieldValues) {
                if ("or".equals(validRangeString) || isFalse(validRangeString)) {
                    continue;
                }
                var validRange = NUMBER_SPLIT.split(validRangeString);

                System.out.println((validRangeString));
                var rangeStartString = validRange[0];
                var rangeEndString   = validRange[1];

                var rangeStart = Short.parseShort(rangeStartString);
                var rangeEnd   = Short.parseShort(rangeEndString);

                validSomewhere.set(rangeStart, rangeEnd + 1);
                validFieldValues.set(rangeStart, rangeEnd + 1);
            }
        }
    }

    static final boolean isFalse(String str) { return str.isEmpty() || str.isBlank(); }

    protected static final String EXAMPLE = "class: 1-3 or 5-7\n" + "row: 6-11 or 33-44\n" + "seat: 13-40 or 45-50\n" + "\n"
            + "your ticket:\n" + "7,1,14\n" + "\n" + "nearby tickets:\n" + "7,3,47\n" + "40,4,50\n" + "55,2,20\n" + "38,6,12";
    protected static final String INPUT   = "departure location: 32-842 or 854-967\n" + "departure station: 44-564 or 582-955\n"
