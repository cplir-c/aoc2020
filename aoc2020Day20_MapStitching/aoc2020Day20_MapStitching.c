...
void findTiles(char tilesString[], char** tilePointerRef) {
    char* charRef = (char*) tilesString;
...
bool constructTileFromTileString(char* tileStringPoint, TileVector* tiles, charVector* strings) {
...
bool buildEdgeMap(TileVector* tiles, fu16ToEdgeReferenceVectorOpenHashMap* edges){
    ...
    for (; tileIndex < contentCount; ++tileIndex, ++tile) {
        ...
        for (; edgeIndex < 4; ++edgeIndex, ++edge) {
            ...
            #endif
            for (fu8 loop = 2; loop > 0; --loop, edgeInteger = edge -> forward) {
                #ifdef DEBUG
